...
也可继续用于其他业务。多 worker 会通过项目租约独占领取：页面异常、验证码
失败、资料提交前 Turnstile 超时或此时停止任务，都会调用 `release` 恢复为
`toClaim`；只有确认邮箱域名被 xAI 永久拒绝才标记为 `failed`。资料提交已经
进入登录/重定向阶段后，邮箱会立即标记为 `done`，即使随后 SSO 超时或用户停止，
也不会再次拿它注册。程序会同时检查收件箱和垃圾邮件，并在领取时记录历史邮件
基线，避免误用旧验证码。

已建号但没有取得 SSO 的邮箱和 Grok 密码会写入权限为 `0600` 的
`accounts/sso_recovery_pending.jsonl`。GUI 会在独立按钮
`重新获取未获取到的SSO 账号（N）` 中显示待处理数量；点击后直接打开 xAI 邮箱
登录页，按“输入邮箱 → 下一步 → 输入密码 → 登录”的流程恢复 SSO，不执行邮箱
API 连通性检查，也不会进入新账号注册。每条记录在一次恢复任务中最多尝试一次，
临时失败会保留供下次点击按钮重试。取得 SSO 后记录立即移出恢复队列，并按现有
流程执行一次 OAuth/Build 交付；已经取得 SSO 但 Build 失败的账号不会自动重试。

Web 登录密码只保存在被 `.gitignore` 忽略且保存时设为 `0600` 的 `config.json`
和运行内存中；日志、`mail_credentials.txt` 及本地租约令牌都不会保存该密码或
服务端 claim token。待恢复文件保存的是 Grok 账号密码而非 OutlookEmail Web
登录密码，同样不会输出到日志。
...
粘性身份。当前账号成功或失败并进入下一个账号后，才会分配新的 Account。
