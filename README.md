...
`accounts/sso_recovery_pending.jsonl`。下次启动注册时，程序先按“登录 → 使用
邮箱登录 → 下一步 → 输入密码 → 登录”的官方页面流程恢复 SSO，每条记录每批
最多尝试一次；临时失败保留到下次启动。取得 SSO 后记录立即移出恢复队列，并按
现有流程执行一次 OAuth/Build 交付；已经取得 SSO 但 Build 失败的账号不会因为
启动注册而自动重试。
